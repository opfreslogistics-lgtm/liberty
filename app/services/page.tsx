'use client'

import { useState } from 'react'
import Image from 'next/image'
import Link from 'next/link'
import AdvancedNavbar from '@/components/AdvancedNavbar'
import Footer from '@/components/homepage/Footer'
import { 
  CreditCard, 
  ArrowRight, 
  Globe, 
  Smartphone,
  Shield,
  CheckCircle2,
  Wallet,
  Clock,
  BarChart3,
  Building2,
  Bell,
  Mail,
  TrendingUp,
  Zap,
  Lock
} from 'lucide-react'

export default function ServicesPage() {
  const services = [
    {
      id: 'debit-cards',
      name: 'Debit & Credit Cards',
      description: 'Secure payment cards with advanced security features and exclusive rewards for everyday purchases.',
      icon: CreditCard,
      image: 'https://images.unsplash.com/photo-1563013544-824ae1b704d3?w=800&h=600&fit=crop',
      features: [
        'Zero liability protection',
        'Cashback rewards programs',
        'Contactless payment enabled',
        'Real-time transaction alerts',
        'Worldwide acceptance',
        '24/7 fraud monitoring'
      ],
      color: 'from-blue-500 to-blue-600',
      href: '/signup',
      popular: true
    },
    {
      id: 'money-transfers',
      name: 'Money Transfers',
      description: 'Send money quickly and securely to anyone, anywhere in the world with competitive rates.',
      icon: ArrowRight,
      image: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=800&h=600&fit=crop',
      features: [
        'Instant domestic transfers',
        'International wire transfers',
        'Low transfer fees',
        'Real-time tracking',
        'Multiple currency support',
        'Scheduled transfers'
      ],
      color: 'from-green-500 to-emerald-600',
      href: '/signup',
      popular: false
    },
    {
      id: 'mobile-banking',
      name: 'Mobile Banking',
      description: 'Bank on the go with our feature-rich mobile app available for iOS and Android devices.',
      icon: Smartphone,
      image: 'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=800&h=600&fit=crop',
      features: [
        'Check balances instantly',
        'Transfer money easily',
        'Pay bills on time',
        'Deposit checks remotely',
        'Biometric authentication',
        'Budget tracking tools'
      ],
      color: 'from-purple-500 to-purple-600',
      href: '/signup',
      popular: false
    },
    {
      id: 'online-banking',
      name: 'Online Banking',
      description: 'Comprehensive online banking platform accessible from any device with full account management.',
      icon: Globe,
      image: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=600&fit=crop',
      features: [
        'Full account management',
        'Transaction history',
        'Bill payment services',
        'Account statements',
        'Transfer between accounts',
        'Secure messaging'
      ],
      color: 'from-orange-500 to-orange-600',
      href: '/signup',
      popular: false
    },
    {
      id: 'bill-pay',
      name: 'Bill Payment Services',
      description: 'Pay all your bills from one convenient location with automatic payment scheduling.',
      icon: Mail,
      image: 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=800&h=600&fit=crop',
      features: [
        'Pay multiple bills at once',
        'Automatic payment scheduling',
        'Payment reminders',
        'Payment history tracking',
        'E-bills available',
        'No monthly fees'
      ],
      color: 'from-red-500 to-red-600',
      href: '/signup',
      popular: false
    },
    {
      id: 'alerts-notifications',
      name: 'Alerts & Notifications',
      description: 'Stay informed about your account activity with real-time alerts and notifications.',
      icon: Bell,
      image: 'https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=800&h=600&fit=crop',
      features: [
        'Balance alerts',
        'Transaction notifications',
        'Payment reminders',
        'Security alerts',
        'Custom alert preferences',
        'Email, SMS, and push notifications'
      ],
      color: 'from-indigo-500 to-indigo-600',
      href: '/signup',
      popular: false
    }
  ]

  const benefits = [
    {
      icon: Shield,
      title: 'Bank-Level Security',
      description: '256-bit SSL encryption and multi-factor authentication protect your transactions'
    },
    {
      icon: Zap,
      title: 'Lightning Fast',
      description: 'Instant transfers and real-time updates for all your banking needs'
    },
    {
      icon: Clock,
      title: '24/7 Availability',
      description: 'Access your accounts and services anytime, anywhere, any day'
    },
    {
      icon: Lock,
      title: 'Secure & Protected',
      description: 'Fraud monitoring and zero liability protection for peace of mind'
    }
  ]

  const features = [
    {
      icon: Smartphone,
      title: 'Mobile First',
      description: 'All services optimized for mobile devices with native apps for iOS and Android.'
    },
    {
      icon: TrendingUp,
      title: 'Real-Time Updates',
      description: 'Get instant notifications and real-time balance updates for all your transactions.'
    },
    {
      icon: BarChart3,
      title: 'Financial Insights',
      description: 'Track spending, set budgets, and analyze your financial habits with detailed reports.'
    },
    {
      icon: Building2,
      title: 'Expert Support',
      description: 'Dedicated customer support team available 24/7 to help with all your banking needs.'
    }
  ]

  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      <AdvancedNavbar />
      
      <main>
        {/* Hero Section */}
        <section className="relative py-20 lg:py-28 overflow-hidden bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
          <div className="absolute inset-0 opacity-10">
            <div className="absolute inset-0" style={{
              backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23000000' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
            }}></div>
          </div>
          
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center max-w-3xl mx-auto">
              <div className="inline-flex items-center space-x-2 px-4 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full text-sm font-semibold mb-6">
                <CreditCard className="w-4 h-4" />
                <span>Banking Services</span>
              </div>
              
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white mb-6 leading-tight">
                Comprehensive{' '}
                <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                  Banking Services
                </span>
              </h1>
              
              <p className="text-xl text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
                Access a full suite of banking services designed to make managing your finances easy, 
                secure, and convenient. From cards to transfers, we've got everything you need.
              </p>
            </div>
          </div>
        </section>

        {/* Services Grid */}
        <section className="py-20 bg-white dark:bg-gray-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                Our Banking Services
              </h2>
              <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                Everything you need to manage your money efficiently
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {services.map((service, index) => {
                const Icon = service.icon
                return (
                  <div
                    key={service.id}
                    className="group relative bg-white dark:bg-gray-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-200 dark:border-gray-700"
                  >
                    {service.popular && (
                      <div className="absolute top-6 right-6 z-10">
                        <span className="px-4 py-1.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs font-bold rounded-full shadow-lg">
                          POPULAR
                        </span>
                      </div>
                    )}
                    
                    <div className="relative h-48 overflow-hidden">
                      <Image
                        src={service.image}
                        alt={service.name}
                        fill
                        className="object-cover group-hover:scale-110 transition-transform duration-700"
                        unoptimized
                      />
                      <div className={`absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-800/50 to-transparent`}></div>
                      <div className="absolute top-6 left-6">
                        <div className={`w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/30`}>
                          <Icon className="w-6 h-6 text-white" />
                        </div>
                      </div>
                    </div>

                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                        {service.name}
                      </h3>
                      <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm leading-relaxed">
                        {service.description}
                      </p>

                      <div className="space-y-2 mb-6">
                        {service.features.slice(0, 4).map((feature, idx) => (
                          <div key={idx} className="flex items-start space-x-2">
                            <CheckCircle2 className="w-4 h-4 text-blue-600 dark:text-blue-400 flex-shrink-0 mt-0.5" />
                            <span className="text-xs text-gray-700 dark:text-gray-300">{feature}</span>
                          </div>
                        ))}
                      </div>

                      <Link
                        href={service.href}
                        className={`inline-flex items-center space-x-2 w-full justify-center px-4 py-3 bg-gradient-to-r ${service.color} text-white rounded-lg font-semibold hover:shadow-lg transform hover:scale-105 transition-all duration-200 text-sm`}
                      >
                        <span>Learn More</span>
                        <ArrowRight className="w-4 h-4" />
                      </Link>
                    </div>
                  </div>
                )
              })}
            </div>
          </div>
        </section>

        {/* Features Section */}
        <section className="py-20 bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-800 dark:to-gray-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                Why Choose Our Services?
              </h2>
              <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                Experience banking services designed with your convenience in mind
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {features.map((feature, index) => {
                const Icon = feature.icon
                return (
                  <div
                    key={index}
                    className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 group"
                  >
                    <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                      <Icon className="w-7 h-7 text-white" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                      {feature.title}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">
                      {feature.description}
                    </p>
                  </div>
                )
              })}
            </div>
          </div>
        </section>

        {/* Benefits Section */}
        <section className="py-20 bg-white dark:bg-gray-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                Trusted by Millions
              </h2>
              <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
                Security and reliability you can count on
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {benefits.map((benefit, index) => {
                const Icon = benefit.icon
                return (
                  <div
                    key={index}
                    className="text-center p-6"
                  >
                    <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
                      <Icon className="w-8 h-8 text-white" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                      {benefit.title}
                    </h3>
                    <p className="text-gray-600 dark:text-gray-400 text-sm">
                      {benefit.description}
                    </p>
                  </div>
                )
              })}
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="py-20 bg-gradient-to-r from-blue-600 to-purple-600 relative overflow-hidden">
          <div className="absolute inset-0 opacity-10">
            <div className="absolute inset-0" style={{
              backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='1'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
            }}></div>
          </div>
          
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6">
              Ready to Get Started?
            </h2>
            <p className="text-xl text-white/90 mb-8 leading-relaxed">
              Open an account today and gain access to all our banking services. 
              Start managing your finances with ease and confidence.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/signup"
                className="inline-flex items-center justify-center space-x-2 px-8 py-4 bg-white text-blue-600 rounded-xl font-semibold text-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-200"
              >
                <span>Open an Account</span>
                <ArrowRight className="w-5 h-5" />
              </Link>
              <Link
                href="/contact"
                className="inline-flex items-center justify-center space-x-2 px-8 py-4 bg-white/10 backdrop-blur-md text-white border-2 border-white/30 rounded-xl font-semibold text-lg hover:bg-white/20 transition-all duration-200"
              >
                <span>Contact Us</span>
              </Link>
            </div>
          </div>
        </section>
      </main>

      <Footer data={{}} />
    </div>
  )
}

